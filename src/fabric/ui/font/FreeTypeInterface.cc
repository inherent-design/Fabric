...
static bool BuildGlyph(FT_Face ft_face, const FontGlyphIndex glyph_index, Rml::Character character,
                       FontGlyphMap& glyphs, const float bitmap_scaling_factor);
...
static void GenerateMetrics(FT_Face ft_face, Rml::FontMetrics& metrics, float bitmap_scaling_factor);
static bool SetFontSize(FT_Face ft_face, int font_size, float& out_bitmap_scaling_factor);
static void BitmapDownscale(Rml::byte* bitmap_new, const int new_width, const int new_height,
                            const Rml::byte* bitmap_source, const int width, const int height, const int pitch,
                            const Rml::ColorFormat color_format);
...
static bool BuildGlyph(FT_Face ft_face, const FontGlyphIndex glyph_index, Rml::Character character,
    ...
    if (error != 0) {
        Rml::Log::Message(Rml::Log::LT_WARNING,
                          "Unable to load glyph at index '%u' on the font face '%s %s'; error code: %d.",
    ...
    if (error != 0) {
        Rml::Log::Message(Rml::Log::LT_WARNING,
                          "Unable to render glyph at index '%u' on the font face '%s %s'; error code: %d.",
                          (unsigned int)glyph_index, ft_face->family_name, ft_face->style_name, error);
        return false;
    }
    ...
    if (!result.second) {
        Rml::Log::Message(Rml::Log::LT_WARNING, "Glyph index '%u' is already loaded in the font face '%s %s'.",
                          (unsigned int)glyph_index, ft_face->family_name, ft_face->style_name);
        return false;
    }
    ...
    if (glyph.bitmap_dimensions.x * glyph.bitmap_dimensions.y != 0) {
        if (ft_glyph->bitmap.pixel_mode != FT_PIXEL_MODE_MONO && ft_glyph->bitmap.pixel_mode != FT_PIXEL_MODE_GRAY &&
            ...
            Rml::Log::Message(Rml::Log::LT_WARNING,
                              "Unable to render glyph on the font face '%s %s': unsupported pixel mode (%d).",
            ...
            Rml::Log::Message(
                Rml::Log::LT_WARNING,
                "Unable to render glyph on the font face '%s %s': bitmap scaling unsupported in mono pixel mode.",
            ...
            glyph.color_format =
                (ft_glyph->bitmap.pixel_mode == FT_PIXEL_MODE_BGRA ? Rml::ColorFormat::RGBA8 : Rml::ColorFormat::A8);

            glyph.bitmap_owned_data.reset(
                new Rml::byte[glyph.bitmap_dimensions.x * glyph.bitmap_dimensions.y * num_bytes_per_pixel]);
            ...
            switch (ft_glyph->bitmap.pixel_mode) {
                ...
                case FT_PIXEL_MODE_BGRA: {
                    if (scale_bitmap) {
                        BitmapDownscale(destination_bitmap, glyph.bitmap_dimensions.x, glyph.bitmap_dimensions.y,
                                        source_bitmap, (int)ft_glyph->bitmap.width, (int)ft_glyph->bitmap.rows,
                                        ft_glyph->bitmap.pitch, glyph.color_format);
                    } else {
                    ...
                    if (glyph.color_format == Rml::ColorFormat::RGBA8) {
                        ...
                        for (int k = 0; k < glyph.bitmap_dimensions.x * glyph.bitmap_dimensions.y * num_bytes_per_pixel;
                             k += 4) {
                            ...
                            for (int c = 0; c < 3; c++)
                                RMLUI_ASSERTMSG(destination_bitmap[k + c] <= alpha,
                                                "Glyph data is assumed to be encoded in premultiplied alpha, but that "
                                                "is not the case.");
#endif
                        }
                    }
...
static void BuildGlyphMap(FT_Face ft_face, int size, FontGlyphMap& glyphs, const float bitmap_scaling_factor,
    ...
    if (load_default_glyphs) {
        ...
        for (FT_ULong character_code = code_min; character_code <= code_max; ++character_code) {
            ...
            if (index != 0 && glyphs.find(index) == glyphs.end())
    ...
    if (it == glyphs.end()) {
        ...
        for (int y = 0; y < glyph.bitmap_dimensions.y; y++) {
            for (int x = 0; x < glyph.bitmap_dimensions.x; x++) {
                ...
                bool near_edge = (x < stroke || x >= glyph.bitmap_dimensions.x - stroke || y < stroke ||
...
static void GenerateMetrics(FT_Face ft_face, Rml::FontMetrics& metrics, float bitmap_scaling_factor) {
    ...
    metrics.underline_position =
        FT_MulFix(-ft_face->underline_position, ft_face->size->metrics.y_scale) * bitmap_scaling_factor / float(1 << 6);
    metrics.underline_thickness =
        FT_MulFix(ft_face->underline_thickness, ft_face->size->metrics.y_scale) * bitmap_scaling_factor / float(1 << 6);
...
static bool SetFontSize(FT_Face ft_face, int font_size, float& out_bitmap_scaling_factor) {
    ...
    if (error != 0) {
        Rml::Log::Message(Rml::Log::LT_ERROR, "Unable to set the character size '%d' on the font face '%s %s'.",
                          font_size, ft_face->family_name, ft_face->style_name);
        return false;
    }
...
static void BitmapDownscale(Rml::byte* bitmap_new, const int new_width, const int new_height,
    ...
    for (int f = 0; f < new_height; f++) {
        ...
        for (int g = 0; g < new_width; g++) {
            ...
            for (int i = 0; i < num_channels; i++)
                bitmap_new[(f * new_width + g) * num_channels + i] =
                    (Rml::byte)Rml::Math::Min(sum[i] * sumscale, 255.f);
        }
    }
}
