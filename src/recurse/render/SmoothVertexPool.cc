...
SmoothVertexPool::~SmoothVertexPool() {
    shutdown();
}

void SmoothVertexPool::init() {
    init(Config{});
}

void SmoothVertexPool::init(const Config& config) {
    if (initialized_)
        return;
    ...
    if (!config_.cpuOnly) {
        ...
        vbh_ = bgfx::createDynamicVertexBuffer(totalVertices, SmoothVoxelVertex::getVertexLayout(),
...
void SmoothVertexPool::shutdown() {
    if (!initialized_)
        return;
...
bool SmoothVertexPool::isValid() const {
    return initialized_;
}

SmoothPoolSlot SmoothVertexPool::allocate(const SmoothVoxelVertex* vertices, uint32_t vertexCount,
    ...
    if (!config_.cpuOnly && vertexCount > 0 && indexCount > 0) {
        bgfx::update(vbh_, slot.vertexOffset,
                     bgfx::copy(vertices, vertexCount * static_cast<uint32_t>(sizeof(SmoothVoxelVertex))));
...
void SmoothVertexPool::free(const SmoothPoolSlot& slot) {
...
bgfx::DynamicVertexBufferHandle SmoothVertexPool::vertexBuffer() const {
    return vbh_;
}

bgfx::DynamicIndexBufferHandle SmoothVertexPool::indexBuffer() const {
    return ibh_;
}

uint32_t SmoothVertexPool::allocatedBuckets() const {
    return allocatedCount_;
}

uint32_t SmoothVertexPool::totalBuckets() const {
    return initialized_ ? config_.initialBuckets : 0;
}

uint32_t SmoothVertexPool::maxVerticesPerBucket() const {
    return initialized_ ? config_.maxVerticesPerBucket : 0;
}

uint32_t SmoothVertexPool::maxIndicesPerBucket() const {
