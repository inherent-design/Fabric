...
class TestResourceFactory {
  public:
    static std::shared_ptr<TestResource> create(const std::string& id) { return std::make_shared<TestResource>(id); }
};
...
TEST_F(ResourceTest, ResourceHubGetResource) {
    if (!fabric::ResourceFactory::isTypeRegistered("test")) {
        fabric::ResourceFactory::registerType<TestResource>(
            "test", [](const std::string& id) { return std::make_shared<TestResource>(id); });
    }
...
TEST_F(ResourceDeterministicTest, ResourceHubMemoryBudget) {
    try {
        ...
        bool operationSucceeded =
            RunWithTimeout([this, testBudget]() { hub_.setMemoryBudget(testBudget); }, std::chrono::milliseconds(500));

        ASSERT_TRUE(operationSucceeded) << "Setting memory budget timed out";

        size_t retrievedBudget = 0;
        operationSucceeded = RunWithTimeout([this, &retrievedBudget]() { retrievedBudget = hub_.getMemoryBudget(); },
        ...
        operationSucceeded = RunWithTimeout([this, originalBudget]() { hub_.setMemoryBudget(originalBudget); },
                                            std::chrono::milliseconds(500));
