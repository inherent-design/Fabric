...
enum class ConnectionState { Disconnected, Connecting, Connected, Draining, Closed };
...
protected:
  void SetUp() override {
    sm = std::make_unique<StateMachine<ConnectionState>>(
      ConnectionState::Disconnected, connectionStateToString);
...
TEST_F(StateMachineTest, TransitionHooksFire) {
  int hookCalls = 0;
  sm->addTransitionHook(ConnectionState::Connecting, ConnectionState::Connected,
                        [&hookCalls]() { hookCalls++; });
...
TEST_F(StateMachineTest, TransitionHookRemoval) {
  int hookCalls = 0;
  auto id = sm->addTransitionHook(ConnectionState::Connecting, ConnectionState::Connected,
                                  [&hookCalls]() { hookCalls++; });
...
TEST_F(StateMachineTest, ThreadSafety) {
  ...
  for (int i = 0; i < 4; ++i) {
    threads.emplace_back([this]() {
      for (int j = 0; j < 100; ++j) {
        try { sm->setState(ConnectionState::Draining); } catch (...) {}
        try { sm->setState(ConnectionState::Connected); } catch (...) {}
      }
    });
  }
...
TEST_F(StateMachineTest, NullHookThrows) {
  ...
  EXPECT_THROW(sm->addTransitionHook(
    ConnectionState::Disconnected, ConnectionState::Connecting, nullptr), FabricException);
}
