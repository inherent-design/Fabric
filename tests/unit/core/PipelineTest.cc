...
TEST(PipelineTest, ExecuteInPriorityOrder) {
  ...
  pipeline.addHandler("second", [](TestContext& ctx, auto next) {
    ctx.log.push_back("B");
    next();
  }, 10);

  pipeline.addHandler("first", [](TestContext& ctx, auto next) {
    ctx.log.push_back("A");
    next();
  }, 1);

  pipeline.addHandler("third", [](TestContext& ctx, auto next) {
    ctx.log.push_back("C");
    next();
  }, 20);
...
TEST(PipelineTest, StableOrderForEqualPriority) {
  Pipeline<TestContext> pipeline;
  pipeline.addHandler("X", [](TestContext& ctx, auto next) {
    ctx.log.push_back("X");
    next();
  }, 0);

  pipeline.addHandler("Y", [](TestContext& ctx, auto next) {
    ctx.log.push_back("Y");
    next();
  }, 0);

  pipeline.addHandler("Z", [](TestContext& ctx, auto next) {
    ctx.log.push_back("Z");
    next();
  }, 0);
...
TEST(PipelineTest, ShortCircuit) {
  Pipeline<TestContext> pipeline;
  pipeline.addHandler("passes", [](TestContext& ctx, auto next) {
    ctx.log.push_back("A");
    next();
  }, 0);

  pipeline.addHandler("blocks", [](TestContext& ctx, auto /*next*/) {
    ctx.log.push_back("B");
    // Does not call next(): short-circuits
  }, 1);

  pipeline.addHandler("skipped", [](TestContext& ctx, auto next) {
    ctx.log.push_back("C");
    next();
  }, 2);
...
TEST(PipelineTest, ContextModificationPropagates) {
  Pipeline<TestContext> pipeline;
  pipeline.addHandler([](TestContext& ctx, auto next) {
    ctx.value = 10;
    next();
  }, 0);
...
TEST(PipelineTest, RemoveNamedHandler) {
  Pipeline<TestContext> pipeline;
  pipeline.addHandler("keep", [](TestContext& ctx, auto next) {
    ctx.log.push_back("kept");
    next();
  }, 0);

  pipeline.addHandler("remove-me", [](TestContext& ctx, auto next) {
...
TEST(PipelineTest, ExceptionPropagation) {
  Pipeline<TestContext> pipeline;
  pipeline.addHandler("ok", [](TestContext& ctx, auto next) {
    ctx.log.push_back("ok");
    next();
  }, 0);

  pipeline.addHandler("boom", [](TestContext&, auto) {
    throw std::runtime_error("handler error");
  }, 1);

  pipeline.addHandler("never", [](TestContext& ctx, auto next) {
