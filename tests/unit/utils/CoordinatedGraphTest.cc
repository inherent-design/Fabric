...
class CoordinatedGraphTest : public ::testing::Test {
  protected:
    ...
    struct TestData {
        ...
        TestData(std::string name = "", int value = 0) : name(std::move(name)), value(value) {}

        // Make TestData copyable since std::atomic is non-copyable by default
        TestData(const TestData& other) : name(other.name), value(other.value), lockCount(other.lockCount.load()) {}
    ...
    bool incrementLockCount(const std::string& key, int amount = 1, bool forWrite = false) {
        return graph->withNode(key, [amount](TestData& data) { data.lockCount += amount; }, forWrite);
    }
...
TEST_F(CoordinatedGraphTest, NodeLocking) {
    ...
    EXPECT_TRUE(graph->withNode("lockTest", [](const TestData& data) { EXPECT_EQ(data.value, 2); }));

    // Test withNode functionality for modifying operation
    EXPECT_TRUE(graph->withNode("lockTest", [](TestData& data) { data.value = 3; }, true));

    // Verify the modification worked
    EXPECT_TRUE(graph->withNode("lockTest", [](const TestData& data) { EXPECT_EQ(data.value, 3); }));
}
...
TEST_F(CoordinatedGraphTest, ProcessDependencyOrder) {
    ...
    for (const auto& key : sortedOrder) {
        EXPECT_TRUE(testGraph->withNode(
            key,
            [&processOrder, &key](TestData& data) {
...
TEST_F(CoordinatedGraphTest, ConcurrentLockOperations) {
    ...
    for (const auto& key : {"A", "B", "C", "D", "E"}) {
        int nodeLockCount = 0;
        EXPECT_NO_THROW(
            graph->withNode(key, [&nodeLockCount](const TestData& data) { nodeLockCount = data.lockCount.load(); }));
...
TEST_F(CoordinatedGraphTest, DeadlockDetectionTest) {
    ...
    // Test 2: Attempt to acquire locks in reverse order (should detect deadlock)
    {
        ...
        EXPECT_THROW({ auto lock1 = graph->tryLockResource("R1", LockMode::Exclusive, 5); }, DeadlockDetectedException);
