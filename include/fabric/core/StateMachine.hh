...
public:
  ...
  void setState(StateEnum state) {
...

    {
      ...
      if (transitions_.count({currentState_, state}) == 0) {
        throwError("Invalid state transition from " +
                   toStringFn_(currentState_) + " to " + toStringFn_(state));
      }
    ...
    FABRIC_LOG_DEBUG("State transition: {} -> {}",
                     toStringFn_(oldState), toStringFn_(state));
  ...
  std::string addHook(StateEnum state, const Hook& hook) {
    ...
    FABRIC_LOG_DEBUG("Added state hook for '{}' with ID '{}'",
                     toStringFn_(state), id);
    return id;
  }
  ...
  std::string addTransitionHook(StateEnum from, StateEnum to, const Hook& hook) {
    ...
    FABRIC_LOG_DEBUG("Added transition hook from '{}' to '{}' with ID '{}'",
                     toStringFn_(from), toStringFn_(to), id);
    return id;
  }
  ...
  bool removeHook(const std::string& hookId) {
    ...
    for (auto& [state, hooks] : stateHooks_) {
      auto it = std::find_if(hooks.begin(), hooks.end(),
                             [&hookId](const HookEntry& e) { return e.id == hookId; });
      if (it != hooks.end()) {
        hooks.erase(it);
...
private:
  ...
  static std::string transitionKey(StateEnum from, StateEnum to) {
    return std::to_string(static_cast<int>(from)) + ":" +
