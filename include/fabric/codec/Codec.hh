...
public:
  ByteReader(const uint8_t* data, size_t size)
      : buf_(data), size_(size), pos_(0) {}

  explicit ByteReader(std::span<const uint8_t> span)
      : buf_(span.data()), size_(span.size()), pos_(0) {}
...
private:
  ...
  const uint8_t* readRaw(size_t n) {
    if (pos_ + n > size_) {
      throwError("ByteReader overrun: requested " + std::to_string(n) +
                 " bytes at offset " + std::to_string(pos_) +
                 " with " + std::to_string(size_ - pos_) + " remaining");
    }
...
public:
  ...
  tryDecode(std::span<const uint8_t> buffer, size_t& consumed) {
    if (buffer.size() < 4) {
      consumed = 0;
      return std::nullopt;
    }
    uint32_t len = static_cast<uint32_t>(buffer[0]) |
