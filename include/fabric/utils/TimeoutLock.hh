 ...
 */
template <typename MutexType> class TimeoutLock {
  public:
    /**
     ...
     */
    static std::optional<std::shared_lock<MutexType>>
    tryLockShared(MutexType& mutex, std::chrono::milliseconds timeout = std::chrono::milliseconds(100)) {
     ...
     * @return An optional containing the lock if successful, or empty if timeout occurred
     */
    static std::optional<std::unique_lock<MutexType>>
    tryLockUnique(MutexType& mutex, std::chrono::milliseconds timeout = std::chrono::milliseconds(100)) {
        // First try to acquire the lock without waiting
     ...
     */
    static std::optional<std::unique_lock<MutexType>>
    tryUpgradeLock(MutexType& mutex, std::shared_lock<MutexType>& sharedLock,
