...
template <typename T, typename KeyType = std::string> class CoordinatedGraph {
  public:
    ...
    std::unique_ptr<ResourceLockHandle> tryLockResource(const KeyType& resourceKey, LockMode mode,
        ...
        auto nodeLock = tryLockNode(resourceKey, intent, forWrite, timeoutMs,
        ...
        // Store the node lock
        {
            ...
            if (lockHistoryEnabled_) {
                lockHistory_.push_back(
                    {"Acquired lock", resourceKey, threadId, std::chrono::steady_clock::now(), mode});
            }
        }
    ...
    bool upgradeResourceLock(const KeyType& resourceKey, std::thread::id threadId = std::this_thread::get_id(),
        ...
        if (!nodeLock || !nodeLock->isLocked()) {
            ...
            auto sharedLock = tryLockNode(resourceKey, LockIntent::Read,
                                          false, // forWrite
                                          timeoutMs, nullptr);
